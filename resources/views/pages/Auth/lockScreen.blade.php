		...
		<div class="cls-content">
			<div class="cls-content-sm panel widget">
				<div class="widget-header bg-primary">
					<h4 class="text-thin">{{ Session::get('lockScreen')['name'] ??'-' }}</h4>
					{{ Session::get('lockScreen')['email'] ??'-' }}
				</div>
				<div class="widget-body">
					<img alt="Profile Picture" class="widget-img img-circle img-border-light"
                                src="{{ !empty(Session::get('lockScreen')['avatar'])?
                                        asset('User/'. Session::get('lockScreen')['avatar']):asset('backend/img/av1.png') }}">
					<p>Vui lòng nhập mật khẩu của bạn để mở khóa màn hình!</p>
					<form action="{{url('admin/lock-screen')}}" method="POST">
                        @csrf
						...
						<div class="form-group text-right">
							<button class="btn btn-block btn-success text-uppercase" type="submit">Đăng nhập</button>
						</div>
					</form>
				</div>
			</div>
			<div class="pad-ver">
				<a href="{{url('admin/login')}}" class="btn-link mar-rgt">Đăng nhập bằng tài khoản khác</a>
			</div>
		</div>
		<!--===================================================-->
